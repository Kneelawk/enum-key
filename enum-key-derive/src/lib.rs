...
pub fn derive_keyable_enum(input: TokenStream) -> TokenStream {
    ...
    let key_enum_ident = format_ident!("{}Key", &enum_ident);

    let variant_cases: Vec<_> = data_enum.variants.iter().map(|var| {
    ...
    let variants: Vec<_> = data_enum.variants.iter().map(|var| var.ident.clone()).collect();
